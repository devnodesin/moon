# Moon API Next - Lean Specification

This document describes the standard response patterns, query options, and aggregation operations for the Moon API. All endpoints follow consistent conventions for success and error responses.

## 1) Overview

This document defines a streamlined Moon API contract optimized for clarity, consistency, and low maintenance cost.

Design goals:

- Keep the endpoint surface small and predictable.
- Preserve AIP-136 custom actions (`/{resource}:action` format).
- Keep transport simple (`GET`, `POST`, `OPTIONS` only).
- Use one shared response and error model across all endpoints.

## 2) Core Rules

1. **Methods:** `GET` (read), `POST` (write/action), `OPTIONS` (CORS preflight only).  
   Other methods return `405 Method Not Allowed`.
2. **Auth:** All endpoints require `Authorization: Bearer <token>` except public endpoints.
3. **Naming:** Collection and field names must follow lowercase snake_case constraints defined in `SPEC.md`.
4. **Identifiers:**  
   - Records, users, and API keys use `id` (ULID)  
   - Collections use `name`
5. **Defaults on schema:** Default values are managed internally and **cannot** be set/updated via collection APIs.
6. **Error body:** Errors always return `{ "message": "..." }` only.

## 3) Data Types

| Type       | API Representation | Notes |
| ---------- | ------------------ | ----- |
| `id`       | string             | Read-only ULID generated by server |
| `string`   | string             | Text |
| `integer`  | number             | 64-bit integer |
| `decimal`  | string             | Decimal values are string-encoded |
| `boolean`  | boolean            | true/false |
| `datetime` | string             | RFC3339 timestamp |
| `json`     | object/array       | Arbitrary JSON |

Aggregation functions (`sum`, `avg`, `min`, `max`) are valid only for `integer` and `decimal`.

## 4) Standard Response Shapes

### 4.1 List Response

```json
{
  "data": [],
  "meta": {
    "count": 15,
    "limit": 15,
    "next": "01KHCZKMM0N808MKSHBNWF464F",
    "prev": null,
    "total": 42
  }
}
```

### 4.2 Single Resource Response

```json
{
  "data": {}
}
```

### 4.3 Mutation Response

```json
{
  "message": "updated successfully",
  "data": [],
  "meta": {
    "success": 1,
    "failed": 0
  }
}
```

### 4.4 Aggregate Response

```json
{
  "data": {
    "op": "sum",
    "field": "price",
    "value": "1299.50"
  }
}
```

## 5) Endpoint Surface (Reduced and Unified)

### 5.1 Public Endpoints (No Auth)

| Endpoint         | Method | Description |
| ---------------- | ------ | ----------- |
| `/`              | GET    | Alias of `/health` (available only when no prefix is configured) |
| `/health`        | GET    | Service health |
| `/doc/`          | GET    | HTML docs |
| `/doc/llms.md`   | GET    | Markdown docs |

### 5.2 Authentication Endpoints

| Endpoint        | Method | Description |
| --------------- | ------ | ----------- |
| `/auth:session` | POST   | Unified session actions: `login`, `refresh`, `logout` |
| `/auth:me`      | GET    | Current authenticated user |
| `/auth:me`      | POST   | Update current user profile/password |

`POST /auth:session` request shape:

```json
{
  "op": "login | refresh | logout",
  "email": "user@example.com",
  "password": "secret",
  "refresh_token": "..."
}
```

- `login` requires `email`, `password`
- `refresh` requires `refresh_token`
- `logout` requires `refresh_token`

### 5.3 Unified Resource Endpoints

| Endpoint               | Method | Description |
| ---------------------- | ------ | ----------- |
| `/{resource}:query`    | GET    | List, get-one, or aggregate depending on query params |
| `/{resource}:mutate`   | POST   | Create, update, destroy, or resource action |
| `/{collection}:schema` | GET    | Read collection schema (data collections only) |

Supported resources:

- System: `users`, `apikeys`, `collections`
- Dynamic data collections: `/{collection}`

Resource capability matrix:

| Resource         | Identifier | Aggregate via `:query` | Schema |
| ---------------- | ---------- | ---------------------- | ------ |
| `users`          | `id`       | No                     | No     |
| `apikeys`        | `id`       | No                     | No     |
| `collections`    | `name`     | No                     | No     |
| `/{collection}`  | `id`       | Yes                    | Yes    |

## 6) `:query` Contract

`GET /{resource}:query` supports three modes:

1. **List mode (default):** no identifier and no `op`
2. **Get-one mode:** `?id=...` (or `?name=...` for collections)
3. **Aggregate mode (data collections only):** `?op=count|sum|avg|min|max` (requires `field` except for `count`)

### Query Options

| Parameter                 | Description |
| ------------------------- | ----------- |
| `limit`                   | Page size (default `15`, max `200`) |
| `after`                   | Cursor for pagination |
| `sort`                    | Comma-separated sort fields (`-field` for desc) |
| `q`                       | Full-text search across text fields |
| `fields`                  | Comma-separated fields to include (`id` is always included) |
| `{field}[eq|ne|gt|lt|gte|lte|like|in]` | Field filters |
| `op`                      | Aggregate op: `count`, `sum`, `avg`, `min`, `max` |
| `field`                   | Target aggregate field (required for non-`count`) |

All filters and query options can be combined.

## 7) `:mutate` Contract

`POST /{resource}:mutate` request shape:

```json
{
  "op": "create | update | destroy | action",
  "data": [],
  "action": "optional-custom-action"
}
```

Rules:

- `data` is always an array (single or batch).
- `create`: objects in `data` must not include system `id`.
- `update`: each object in `data` must include identifier (`id` or `name`).
- `destroy`: each object in `data` must include identifier only.
- `action`: use `action` plus optional payload fields.

Status behavior:

- `201 Created` when `op=create` and at least one record is created.
- `200 OK` for `update`, `destroy`, `action` with at least one successful operation.
- Partial success is allowed for batch writes; report counts via `meta.success` and `meta.failed`.

## 8) Resource-Specific Operations

### 8.1 Collections (`/collections:mutate`)

- `op=create`: create collection(s) with `name` and `columns`
- `op=update`: schema changes using:
  - `add_columns`
  - `rename_columns`
  - `modify_columns`
  - `remove_columns`
- `op=destroy`: delete collection(s) by `name`

**Restriction:** Requests containing `default` in column mutations must be rejected (`400`).

### 8.2 Users (`/users:mutate`)

- Standard `create`, `update`, `destroy`
- `op=action` supported actions:
  - `reset_password`
  - `revoke_sessions`

### 8.3 API Keys (`/apikeys:mutate`)

- Standard `create`, `update`, `destroy`
- `op=action` supported action:
  - `rotate`

`key` values are returned only on creation/rotation response and must not be retrievable later.

### 8.4 Data Records (`/{collection}:mutate`)

- Standard `create`, `update`, `destroy`
- Optional custom `op=action` for collection-specific server actions

## 9) Error Model

Allowed status codes:

- `200` OK
- `201` Created
- `400` Bad Request
- `401` Unauthorized
- `404` Not Found
- `405` Method Not Allowed
- `429` Too Many Requests
- `500` Internal Server Error

Error response shape:

```json
{
  "message": "human-readable error"
}
```

No additional error fields are used.

## 10) Security and Operational Headers

### 10.1 Authentication Types

- JWT access token (user flows)
- API key (service flows)
- Both use `Authorization: Bearer <token>`

### 10.2 Rate Limiting Headers

- `X-RateLimit-Limit`
- `X-RateLimit-Remaining`
- `X-RateLimit-Reset`
- `Retry-After` (on `429`)

### 10.3 Request Correlation

- Incoming `X-Request-ID` is accepted.
- Response always returns `X-Request-ID`.
